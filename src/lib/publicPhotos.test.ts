...
vi.mock("firebase/storage", () => ({
  ...
  uploadBytesResumable: vi.fn(() => ({
    on: (_evt: any, _prog: any, _err: any, complete: any) =>
      complete && complete(),
  })),
}));
...
describe("publicPhotos helpers", () => {
  ...
  it("pickThumb selects preferred size", async () => {
    ...
    const res1 = pickThumb({
      id: "x",
      categoryId: "c",
      createdAt: 1,
      sizes: { "960": { jpg: "a.jpg", width: 960, height: 600 } },
    } as any);
    expect(res1.src).toBe("a.jpg");
    const res2 = pickThumb({
      id: "x",
      categoryId: "c",
      createdAt: 1,
      sizes: { "640": { jpg: "b.jpg", width: 640, height: 400 } },
    } as any);
  ...
  it("listPublicPhotos falls back to Firestore when API fails", async () => {
    ...
    fsMock.getDocs.mockResolvedValueOnce(
      snap([
        { id: "1", createdAt: 3 },
        { id: "2", createdAt: 2 },
      ]),
  ...
  it("uploadMasterAndCreateProcessingDoc: success path uses API POST", async () => {
    const { uploadMasterAndCreateProcessingDoc } =
      await import("./publicPhotos");
    const fetchSpy = vi
      .spyOn(globalThis, "fetch" as any)
      .mockResolvedValue({
        ok: true,
        json: async () => ({ ok: true, id: "new" }),
      } as any);
    const file = new File([new Uint8Array([1, 2])], "photo.jpg", {
      type: "image/jpeg",
    });
    const res = await uploadMasterAndCreateProcessingDoc({
      file,
      categoryId: "cat",
    });
  ...
  it("uploadMasterAndCreateProcessingDoc: throws when API fails", async () => {
    const { uploadMasterAndCreateProcessingDoc } =
      await import("./publicPhotos");
    vi.spyOn(globalThis, "fetch" as any).mockResolvedValue({
      ok: false,
    ...
    const file = new File([new Uint8Array([1, 2])], "photo.png", {
      type: "image/png",
    });
    await expect(
      uploadMasterAndCreateProcessingDoc({ file, categoryId: "cat" }),
    ).rejects.toThrow("bad");
  });
