...
vi.mock("@/lib/admin/sequence", () => ({
  findPublicMaxSequence: vi.fn().mockResolvedValue(0),
}));
...
describe("DELETE/POST /api/public-photos/delete", () => {
  ...
  it("DELETE uses id param and handles missing", async () => {
    const mod = await import("./route");
    const resMissing = await mod.DELETE(
      new Request("http://test?foo=bar", { method: "DELETE" }),
    );
    expect(resMissing.status).toBe(400);
    refApi.get.mockResolvedValueOnce({ exists: false });
    const res = await mod.DELETE(
      new Request("http://test?id=p1", { method: "DELETE" }),
    );
    expect(res.status).toBe(204);
  });
});
