...
export default function OrderDownloadPage() {
  ...
  useEffect(() => {
    ...
    let aborted = false;
    fetch(`/api/session-orders/${orderId}?token=${token}`, {
      cache: "no-store",
    })
      .then((res) => res.json())
      .then((payload) => {
  ...
  const triggerDownload = useCallback(async () => {
    ...
    doneTimeoutRef.current = setTimeout(() => {
      ...
      setProgress(100);
      setElapsedMs(
        startedAtRef.current ? Date.now() - startedAtRef.current : elapsedMs
      );
      setState("done");
  ...
  const remainingMs =
    state === "running" && estimatedMs
      ? Math.max(estimatedMs - elapsedMs, estimatedMs * 0.15)
      : null;
    ...
    <div className="min-h-screen bg-[#030303] text-gray-100">
      ...
      <main className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-4 pb-16 pt-10 text-center sm:px-6 lg:px-8">
        ...
        <p className="text-sm text-white/70">
          Estamos a gerar o ZIP com as fotos escolhidas. Depois clica em
          “Descarregar”.
        </p>
        ...
        <div className="w-full rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_25px_120px_rgba(0,0,0,0.45)] backdrop-blur-sm">
          {state === "running" ? (
            <div className="space-y-3 text-left">
              <p className="text-sm text-white/80">
                {t("preparingFile")}
                {selectedCount
                  ? ` ${t("photosCount", { count: selectedCount })}`
                  : ""}
                …
              </p>
              ...
              <div className="flex flex-col gap-1 text-xs text-white/60 sm:flex-row sm:items-center sm:justify-between">
                <span>
                  {t("progress", {
                    percent: Math.min(100, Math.max(0, progress)).toFixed(0),
                  })}
                </span>
                <span>
                  {remainingMs
                    ? t("estimatedTime", {
                        total: formatDuration(estimatedMs || 0),
                        remaining: formatDuration(remainingMs),
                      })
                    : estimatedMs
                    ? t("estimatedTimeOnly", {
                        total: formatDuration(estimatedMs),
                      })
                    : t("estimating")}
                </span>
              </div>
          ...
          {state === "done" ? (
            <div className="space-y-2 text-left">
              <p className="text-sm text-emerald-300">{t("fileReady")}</p>
              {elapsedMs ? (
          ...
          {state === "done" && downloadState === "downloading" ? (
            <div className="mt-4 space-y-2 text-left">
              <p className="text-sm text-white/80">{t("downloading")}</p>
              <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-white via-white/90 to-white/70 transition-[width]"
              ...
              <p className="text-xs text-white/60">
                {t("estimatedProgress", { percent: downloadProgress })}
              </p>
            </div>
          ) : null}
        </div>
