...
const NavBar: React.FC = () => {
  ...
  const loginHref = `/${locale}/login?callbackUrl=${encodeURIComponent(`/${locale}/admin`)}`;
  ...
  return (
    <div className="navbar bg-base-100">
      <div className="navbar-start">
        ...
        <span className="text-xl pl-2 pt-1">Momentos</span>
      </div>
      ...
      <div className="navbar-end">
        {isAuthenticated ? (
          <button type="button" onClick={handleLogout} className="btn btn-ghost mr-2 text-sm">
            {translate("logout")}
          </button>
        ) : (
          <Link href={loginHref} className="btn btn-ghost mr-2 text-sm">
            Entrar
          </Link>
        )}
        <div className="dropdown dropdown-end">
          ...
          <ul tabIndex={0} className="menu menu-sm dropdown-content bg-base-100 rounded-box z-10 mt-2 w-28 p-2 shadow">
            <li>
              <Link href={hrefWithQs} locale="pt" className={locale === "pt" ? "active" : undefined} aria-current={locale === "pt" ? "page" : undefined}>
                PT
              </Link>
            </li>
            <li>
              <Link href={hrefWithQs} locale="en" className={locale === "en" ? "active" : undefined} aria-current={locale === "en" ? "page" : undefined}>
                EN
              </Link>
            </li>
          </ul>
